...
<html>
  <head>
    ...
    <title>Low End Calendar-Submit Request For Editing Permissions Removal</title>
	...
	if(isset($_POST['s']) && $_POST['s'] == "y")
	{
		...
		if(isset($_SESSION['username']) && $_SESSION['username'] != "")
		{
			...
			if(isset($user[2]) && $user[2] >= 2)
			{
				...
				$debug=insert_ticket($db,$user,"Revoke edit rights",$details);
				if($debug === true)
				{
  ...
  <h1>Low End Calendar-Submit Request For Editing Permissions Removal</h1>
  <form method="post" action="reqnoedit.php">
  <input type="hidden" name="s" value="y">
  <input type="hidden" name="user" value="<?php echo $user; ?>">
  Describe why you no longer wish to edit this calendar:<br>
