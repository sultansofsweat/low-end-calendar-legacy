...
<html>
  <head>
    ...
    <title>Low End Calendar-Add Event</title>
  ...
  <?php
	...
	if(isset($_POST['s']) && $_POST['s'] == "y")
	{
		...
		if(isset($_SESSION['username']) && $_SESSION['username'] != "")
		{
			...
			{
				//Begin submission
				$infos=array(false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false);
				if(isset($_POST['name']))
				{
				...
				if(isset($_POST['repeat']) && $_POST['repeat'] == "yes")
				{
					...
					{
						$infos[16]=preg_replace("/[^A-Za-z0-9]/","",$_POST['rmonth']);
					}
					if(isset($_POST['rday']))
					{
						$infos[17]=preg_replace("/[^0-9]/","",$_POST['rday']);
					}
					if(isset($_POST['ryear']))
					{
						$infos[18]=preg_replace("/[^0-9]/","",$_POST['ryear']);
					}
				}
				...
				if(check_if_valid($infos[0],$infos[2],$infos[3],$infos[4],$infos[5],$infos[6],$infos[7],$infos[8],$infos[10],$infos[9],$infos[12]) === true && $infos[1] !== false && check_valid_repeat($infos[11],$infos[16],$infos[17],$infos[18]) !== false)
				{
					$stime=strtotime($infos[2] . " " . $infos[3] . ", " . $infos[4] . " " . $infos[5] . ":" . $infos[6] . " " . $infos[7]);
					if($infos[11] !== false)
					{
						$repeat=$infos[11] . "," . strtotime($infos[16] . " " . $infos[17] . " " . $infos[18]);
					}
					else
					{
						$repeat="";
					...
					$debug=insert_event($db,$infos[0],$infos[1],$infos[14],$stime,$etime,$infos[8],$infos[15],$infos[13],$private,time(),$repeat);
					if($debug === true)
					{
  ...
  <h1>Low End Calendar-Add Event</h1>
  <form method="post" action="addevent.php">
  <input type="hidden" name="s" value="y">
  Name: <input type="text" name="name" value="<?php if(isset($_POST['name'])) { echo stripcslashes($_POST['name']); } ?>"><br>
	...
	if(count($users) > 0)
	{
		foreach($users as $user)
		{
			if(isset($user[0]) && $user[0] != "" && isset($user[1]) && $user[1] != "")
			{
				...
				$sel=false;
				if(isset($_POST['owner']) && $_POST['owner'] == $user[0])
				{
					echo "selected=\"selected\"";
				}
				if(isset($_SESSION['username']) && $_SESSION['username'] == $user[0] && !isset($_POST['owner']))
				{
					echo "selected=\"selected\"";
				}
  ...
  Start time: <input maxlength="2" size="2" type="text" name="shour" value="<?php if(isset($_POST['shour'])) { echo $_POST['shour']; } else { echo date("g"); } ?>"><input maxlength="2" size="2" type="text" name="sminute" value="<?php if(isset($_POST['sminute'])) { echo $_POST['sminute']; } else { echo date("i"); } ?>"><select name="smerid">
  <option value="AM" <?php if(isset($_POST['smerid']) && $_POST['smerid'] == "AM") { echo "selected=\"selected\""; } elseif(date("A") == "AM") { echo "selected=\"selected\""; } ?>>AM</option>
  <option value="PM" <?php if(isset($_POST['smerid']) && $_POST['smerid'] == "PM") { echo "selected=\"selected\""; } elseif(date("A") == "PM") { echo "selected=\"selected\""; } ?>>PM</option>
  </select><br>
  Duration: <input type="checkbox" name="allday" value="yes" <?php if(isset($_POST['allday']) && $_POST['allday'] == "yes") { echo "checked=\"checked\""; } ?>> All day or <input type="text" size="4" name="duration" value="<?php if(isset($_POST['duration'])) { echo $_POST['duration']; } else { echo 3; } ?>"><select name="dmod">
  <option value="min" <?php if(isset($_POST['dmod']) && $_POST['dmod'] == "min") { echo "selected=\"selected\""; } ?>>Minutes</option>
  <option value="hr" <?php if(!isset($_POST['dmod']) || (isset($_POST['dmod']) && $_POST['dmod'] == "hr")) { echo "selected=\"selected\""; } ?>>Hours</option>
  <option value="day" <?php if(isset($_POST['dmod']) && $_POST['dmod'] == "day") { echo "selected=\"selected\""; } ?>>Days</option>
  </select><br>
  <input type="checkbox" name="repeat" value="yes" <?php if(isset($_POST['repeat']) && $_POST['repeat'] == "yes") { echo "checked=\"checked\""; } ?>> Repeat every <input type="text" name="frequency" value="<?php if(isset($_POST['frequency'])) { echo $_POST['frequency']; } else { echo "1"; } ?>"> week(s) until <input maxlength="9" size="9" type="text" name="rmonth" value="<?php if(isset($_POST['rmonth'])) { echo $_POST['rmonth']; } elseif(isset($rmonth)) { echo $rmonth; } else { echo date("F"); } ?>"><input maxlength="2" size="2" type="text" name="rday" value="<?php if(isset($_POST['rday'])) { echo $_POST['rday']; } elseif(isset($rday)) { echo $rday; } else { echo date("j"); } ?>"><input maxlength="4" size="4" type="text" name="ryear" value="<?php if(isset($_POST['ryear'])) { echo $_POST['ryear']; } elseif(isset($ryear)) { echo $ryear; } else { echo date("Y"); } ?>"> (month MUST be in text format!)<br>
  Display publicly: <input type="radio" name="public" value="yes" <?php if(isset($_POST['public']) && $_POST['public'] == "yes") { echo "checked=\"checked\""; } ?>>Yes | <input type="radio" name="public" value="no" <?php if(!isset($_POST['public']) || (isset($_POST['public']) && $_POST['public'] != "yes")) { echo "checked=\"checked\""; } ?>>No<br>
  Participants: <input type="text" name="participants" value="<?php if(isset($_POST['participants'])) { echo $_POST['participants']; } ?>"><br>
  ...
  Location: <input type="text" name="location" value="<?php if(isset($_POST['location'])) { echo $_POST['location']; } ?>"> (Usually a URL, spaces will be removed)<br>
  Description:<br>
  <textarea rows="10" cols="50" name="description"><?php if(isset($_POST['description'])) { echo stripcslashes($_POST['description']); } ?></textarea><br>
