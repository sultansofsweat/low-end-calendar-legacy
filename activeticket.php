...
<html>
  <head>
    ...
    <title>Low End Calendar-Mark Ticket As Active</title>
  ...
  <?php
	...
	if(isset($_POST['s']) && $_POST['s'] == "y" && isset($_POST['id']) && isset($_POST['user']) && $_POST['user'] != "" && isset($_POST['description']) && $_POST['description'] != "")
	{
		...
		if($id != "" && $id > 0 && $user != "" && $user >= 0)
		{
			...
			$debug=update_ticket($db,$id,$user,1,$details);
			if($debug === true)
			{
  ...
  <h1>Low End Calendar-Mark Ticket As Active</h1>
  <form method="post" action="activeticket.php">
