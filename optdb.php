...
<html>
  <head>
    ...
    <title>Low End Calendar-Optimize Database</title>
  ...
  <?php
	...
	if(isset($_POST['s']) && $_POST['s'] == "y")
	{
		...
		if(isset($_SESSION['username']) && $_SESSION['username'] != "")
		{
			...
			{
				//Begin submission
				$debug=optimize_database($db);
				if($debug === true)
				{
					echo ("<script type=\"text/javascript\">window.location = \"index.php?odb=yes\"</script>");
				}
				else
				{
					echo ("<script type=\"text/javascript\">window.location = \"index.php?odb=no\"</script>");
				}
			}
			else
			{
  ...
  <h1>Low End Calendar-Optimize Database</h1>
  <form method="post" action="optdb.php">
  <input type="hidden" name="s" value="y">
  Are you sure you want to optimize the database?<br>
