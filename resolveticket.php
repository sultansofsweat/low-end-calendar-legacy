...
<html>
  <head>
    ...
    <title>Low End Calendar-Mark Ticket As Resolved</title>
  ...
  <?php
	...
	if(isset($_POST['s']) && $_POST['s'] == "y" && isset($_POST['id']) && isset($_POST['user']) && $_POST['user'] != "" && isset($_POST['description']) && $_POST['description'] != "")
	{
		...
		if($id != "" && $id > 0 && $user != "" && $user >= 0)
		{
			...
			$debug=update_ticket($db,$id,$user,3,$details);
			if($debug === true)
			{
  ...
  <h1>Low End Calendar-Mark Ticket As Resolved</h1>
  <form method="post" action="resolveticket.php">
  <input type="hidden" name="s" value="y">
  <input type="hidden" name="id" value="<?php echo $id; ?>">
  <input type="hidden" name="user" value="<?php echo $user; ?>">
  Reason for marking as resolved:<br>
