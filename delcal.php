...
<html>
  <head>
    ...
    <title>Low End Calendar-Delete All Calendar Events</title>
	...
	if(isset($_POST['s']) && $_POST['s'] == "y")
	{
		...
		if(isset($_SESSION['username']) && $_SESSION['username'] != "")
		{
			...
			if(isset($user[2]) && $user[2] >= 4)
			{
				...
				if($errors == 0)
				{
					echo ("<script type=\"text/javascript\">window.location = \"index.php?cdb=yes\"</script>");
				}
				else
				{
					echo ("<script type=\"text/javascript\">window.location = \"index.php?cdb=no\"</script>");
				}
			}
			else
			{
  ...
  <h1>Low End Calendar-Clear Calendar</h1>
  <form method="post" action="delcal.php">
  <input type="hidden" name="s" value="y">
  Are you sure you want to delete all events?<br>
